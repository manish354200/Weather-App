...
async function getdata(cityName) {
 const promise = await fetch(`http://localhost:5000/weather?city=${cityName}`)
...
button.addEventListener("click", async () => {
  try {
    ...
    alert("Failed to fetch weather data. Make sure the backend is running on localhost:5000");
  }
})